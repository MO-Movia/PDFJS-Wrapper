...
describe('CommentPopoverComponent', () => {
    ...
    it('should handle handleKeydown', () => {
        expect(component.handleKeydown({ key: 'Tab', preventDefault: () => { } } as unknown as KeyboardEvent, 1)).toBeFalsy();
    });
    it('should handle handleKeydown when event.key === ArrowDown', () => {
        expect(component.handleKeydown({ key: 'ArrowDown', preventDefault: () => { }, stopPropagation: () => { } } as unknown as KeyboardEvent, 1)).toBeFalsy();
    });
    it('should handle handleKeydown when event.key === ArrowUp', () => {
        expect(component.handleKeydown({ key: 'ArrowUp', preventDefault: () => { }, stopPropagation: () => { } } as unknown as KeyboardEvent, 1)).toBeFalsy();
    });
    ...
    it('should handle checkedTag', () => {
        ...
        component.checkedTag({ isChecked: false, isPrivate: true, id: 1, name: 'test' });
        expect(spy).toHaveBeenCalled();
    });
    it('should handle tagSearch', () => {
        component.allTags = [{ isChecked: false, isPrivate: true, id: 1, name: 'test' }, { isChecked: false, isPrivate: true, id: 2, name: 'test' }];
